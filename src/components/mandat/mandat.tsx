                    ...
                    {mandatDetails.ressources.map((ressource) => (
                      <div key={ressource.id} className="flex items-center space-x-3 p-3 border rounded-lg">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-muted-foreground">Quantité: {ressource.quantite}</p>
                          {ressource.description && (
                            <p className="text-xs text-muted-foreground mt-1">{ressource.description}</p>
                          )}
                        </div>
                      </div>
                    ))}
...
function MandatsList() {
  ...
  const handleAddSteps = (id: string) => {
    toast({
      ...
      description: `Ajout d'étapes pour le mandat ${id}`,
    })
    // Implement add steps functionality
    // You could navigate to a steps management page or open a modal
  }

  const handleAddAttachments = (id: string) => {
    toast({
      title: "Ajouter des pièces jointes",
      description: `Ajout de pièces jointes pour le mandat ${id}`,
    })
    // Implement add attachments functionality
    // You could open a file upload modal or navigate to an attachments page
  }
                  ...
                  <TableRow key={mandat.id}>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent>
                          <DropdownMenuItem onClick={() => window.open(`/auth/mandats/${mandat.id}`, "_blank")}>
