...
export default function Etapes() {
  ...
  const fetchVilles = async () => {
    try {
      const response = await fetch("/api/villes/all")
  ...
  const handleCreateEtape = async () => {
    ...
    try {
      setLoading(true)
      const response = await fetch("/api/etapes/create", {
        method: "POST",
        headers: {
  ...
  const handleUpdateEtape = async () => {
    ...
    try {
      setLoading(true)
      const response = await fetch(`/api/etapes/${selectedEtape.id}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const validateForm = () => {
    ...
    if (formData.mandatId === 0) {
      toast({
        title: "Erreur de validation",
        description: "Le mandat est obligatoire",
        variant: "destructive",
      })
      return false
    }
    ...
    <div className="container mx-auto py-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Gérez les étapes de vos mandats</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
              ...
              <TableBody>
                {etapes.map((etape) => (
                  <TableRow key={etape.id}>
                    ...
                    <TableCell>
                      {etape.mandat ? (
                        <Badge variant="outline">{etape.mandat.reference}</Badge>
                      ) : (
